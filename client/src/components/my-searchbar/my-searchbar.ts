...
import {Config, Ion, Platform} from "ionic-angular";
...
@Component({
    ...
    '</div>' +
    '<button ion-button #cancelButton mode="ios" [tabindex]="_isActive ? 1 : -1" clear (click)="cancelSearchbar($event)" (mousedown)="cancelSearchbar($event)" class="my-searchbar-ios-cancel" type="button">{{cancelButtonText}}</button>',
    host: {
...
export class MySearchbar extends Ion {
    ...
    constructor(
        ...
        super(config, elementRef, renderer, 'my-searchbar');
    ...
    cancelSearchbar(ev: UIEvent) {
        ...
        //after clicking cancel button,search bar wil not be cleared
        // this.clearInput(ev);
        this._shouldBlur = true;
